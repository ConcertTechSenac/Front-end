  ...
  const enviar = async () => {
    ...
    try {
      ...
      const catalogoTexto = produtos.map((p) =>
        `- ${p.nome} | Categoria: ${p.categoria} | Preço: R$${p.preco.toFixed(2)} | Estoque: ${p.estoque ?? 0} unid${p.destaque ? ' | DESTAQUE' : ''}`
      ).join('\n');

      const contexto = `Você é o assistente virtual da loja Hard Tech, especializada em tecnologia. Responda sempre em português, de forma amigável e objetiva. Não invente produtos fora da lista abaixo.\n\nCatálogo atual:\n${catalogoTexto}`;
